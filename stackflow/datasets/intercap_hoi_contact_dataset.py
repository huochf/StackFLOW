...
from stackflow.datasets.intercap_metadata import (
    ...
    get_image_path,
    get_person_mask_path,
    get_object_mask_path,
)


class IntercapContactDataset(object):

    def __init__(self, cfg, exlusion_occlusion=False, is_train=True):
        self.train = is_train
        self.smpl = SMPLX(model_path=cfg.data.smplx_path, gender='male', ext='pkl')

        if is_train:
            data_list = load_pickle(cfg.dataset.intercap.datalist_train)
        else:
            data_list = load_pickle(cfg.dataset.intercap.datalist_test)
        ...
        self.subsampling_indices = load_pickle('data/intercap/subsampling_points.pkl')
    ...
    def __getitem__(self, idx):
        ...
        img_id = item['img_id']
        image_file = get_image_path(img_id)
        image = self.load_image(image_file)

        mask = None
        if np.random.random() < self.bg_aug_ratio:
            mask_path = get_person_mask_path(img_id)
            ...
            mask_path = get_object_mask_path(img_id)
        ...
        img_patch, img_size, box_centers, scale = self.get_example(image, mask, center_x, center_y, bbox_size, bbox_size,
            self.img_size, self.img_size, self.mean, self.std, self.train, self.augm_config)
        ...
        obj_name = OBJECT_IDX2NAME[int(img_id.split('_')[1]) - 1]
        ...
        object_vertices_mask = np.zeros(2500, dtype=np.float32)
...
def subsampling():
    ...
    for obj_name in object_templates.keys():
        ...
        while not done:
            sample_points_l1, _ = trimesh.sample.sample_surface_even(mesh, 256, radius=0.002)
    ...
    save_pickle(results, 'data/intercap/subsampling_points.pkl')
