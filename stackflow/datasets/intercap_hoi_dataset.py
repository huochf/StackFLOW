...
class InterCapDataset(object):
    ...
    def __init__(self, cfg, is_train=True, for_evaluation=False):
        ...
        self.imgid2anno = {item['img_id']: item for item in self.annotation_list}
    ...
    def get_item_for_training(self, idx):
        ...
        if np.isnan(annotation['smplx_betas_neutral']).any():
        ...
        person_joint_3d = annotation['smplx_joints_3d'][:22]
        ...
        person_joint_2d = annotation['smplx_joints_2d'][:22]
        ...
        smplh_pose_rotmat = annotation['smplx_pose_rotmat']
        ...
        rot_center_x = annotation['smplx_joints_2d'][0, 0]
        rot_center_y = annotation['smplx_joints_2d'][0, 1]
        ...
        smpl_trans_3d = annotation['smplx_joints_3d'][0]
        ...
        results['smpl_betas'] = annotation['smplx_betas_neutral'].astype(np.float32) # [10, ]=
        ...
        results['object_labels'] = self.dataset_metadata.OBJECT_NAME2LABEL[obj_name]
    ...
    def get_item_for_evaluation(self, idx):
        annotation = self.annotation_list[idx]
        results = self.load_item(annotation['img_id'])

        return results


    def load_item(self, img_id):
        annotation = self.imgid2anno[img_id]

        img_id = annotation['img_id']
        obj_name = self.dataset_metadata.OBJECT_IDX2NAME[img_id.split('_')[1]]
